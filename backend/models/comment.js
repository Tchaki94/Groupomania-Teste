...
Comment.getAllComment = (callback) => {
    ...
    connection.query(`SELECT c.id, c.date_pub, c.comment, p.id as 'postId', u.name as 'username', u.id as  'userId', u.image as 'userimage' FROM comments c INNER JOIN post p on c.post_id = p.id INNER JOIN users u on c.user_id = u.id`, (err, res) => {
        if ( err ) {
            throw err
        }
